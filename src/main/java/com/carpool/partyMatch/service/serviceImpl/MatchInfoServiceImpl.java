...
//    }
    ...
    public void cancelMatchInfo(MatchInfoDto matchInfoDto){
        ...
        //파티 상태 확인
        MatchInfo matchInfo = matchInfoRepository.findAllByPartyInfoIdAndUserIdOrderByIdDesc(matchInfoDto.getPartyInfoId(), matchInfoDto.getUserId()).stream().findFirst().orElse(null);
        if(matchInfo == null){
            throw new ApiException(ApiStatus.NOT_EXIST_MATCH);
        }
...
//        }
    ...
    public void cancelMatchInfoRollback(MatchCancelled matchCancelled){
        ...
        //파티 상태 확인
        MatchInfo matchInfo = matchInfoRepository.findAllByPartyInfoIdAndUserIdOrderByIdDesc(matchCancelled.getPartyInfoId(), matchCancelled.getUserId()).stream().findFirst().get();
    ...
    public MatchInfo acceptMatchInfo(MatchProcessDto matchProcessDto){
        ...
        MatchInfo matchInfo =  matchInfoRepository.findAllByPartyInfoIdAndUserIdOrderByIdDesc(matchProcessDto.getPartyInfoId(), matchProcessDto.getUserId()).stream().findFirst().get();
...
//        matchAccepted.publish();
    ...
    public MatchInfo acceptMatchInfoRollback(MatchAccepted matchAccepted){
        ...
        MatchInfo matchInfo =  matchInfoRepository.findAllByPartyInfoIdAndUserIdOrderByIdDesc(matchAccepted.getPartyInfoId(), matchAccepted.getUserId()).stream().findFirst().get();
